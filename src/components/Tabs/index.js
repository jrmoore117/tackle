...
const TabLabel = ({
  ...
  return (
    <div
      ...
      className={
        active ? `tab tab--active--${color}` : `tab tab--inactive--${color}`
      }
    >
      {newTab && (
        <Icon
          ...
          variant="clickable"
          className={
            hover && multipleTabs ? "tabdelete-hover" : "tabdelete-default"
          }
...
export const Tabs = ({ index, color, newTab, onClick, children, ...props }) => {
  const defaultChildrenCopyState =
...
export const Tab = ({ icon, tabTitle, children, ...props }) => {
