...
export const Menu = ({ color, children, className, withLinks, ...props }) => {
  ...
  if (withLinks) {
    return (
      <Link
        onClick={onClick}
        className={`menuitem ${
          isActive ? `menuitem--active--${color}` : "menuitem--inactive"
        } ${className || ""}`}
        {...props}
      >
        {label}
...
export const MenuCategory = ({ label, onClick, className, ...props }) => (
  <div
    onClick={onClick}
