...
const MultiSelect = ({
  ...
  const uniqueItems = Array.from(new Set(items.map((item) => item.label))).map(
    (label) => {
  ...
  const handleOnKeyUp = (e) => {
    if (e.key === "Enter") {
      const searchStringMatch = options.find(
        (item) => item.label.toLowerCase() === searchString.toLocaleLowerCase()
      );
        ...
        <span className="selected-items">
          {selected.map((item, i) => (
            <Chip
              ...
              onClick={() =>
                handleSetSelected(
                  selected.filter((i) => i.label !== item.label)
                )
              }
      ...
      <ul className={isOpen ? "item-list" : "hidden"}>
        {options.map((item, i) => (
          <MultiSelectItem
            ...
            label={item.label}
            isSelected={
              selected.find((i) => i.label === item.label) === undefined
                ? false
                : true
            }
            addItem={() => handleSetSelected([...selected, item])}
            removeItem={() =>
              handleSetSelected(selected.filter((i) => i.label !== item.label))
            }
          />
        ))}
        {options.length === 0 && (
